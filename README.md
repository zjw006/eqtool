...
| 文件 | 页面 | 说明 |
|------|------|------|
| `pick-theorem.html` | 皮克定理动画课堂 | 格点多边形面积公式 S = I + B/2 − 1 的交互动画演示：第一部分逐个数内部格点和边界格点，用鞋带公式验证结果一致；第二部分用欧拉公式逐步剖分证明皮克定理；第三部分果农数树、铺地砖两个生活应用题 |
...
| 文件 | 页面 | 说明 |
|------|------|------|
| `fields-medal-lab.html` | 菲尔兹奖理论 · 动画实验室 | 二十项菲尔兹奖获奖理论，每项只挑一个最直观的应用做成可动手玩的动画。涵盖物理宇宙（拓扑绝缘体、规范场、弦论、量子场论、共形对称）、流体与交通（纳维-斯托克斯方程、最优传输理论）、密码学（椭圆曲线、格密码）、材料相变（相变理论、拓扑相）、AI与影像（蒙日-安培方程、最优传输与GAN）、机器人与经济（刚体控制、博弈论纳什均衡）。星空粒子背景 + 金色奖章主题 + 暗色宇宙风格，保留原始暗色主题但使用二次元字体（ZCOOL KuaiLe + Quicksand）|
...
├── pick-theorem.html                   # 皮克定理动画课堂 (格点几何, 欧拉公式证明)
...
├── fields-medal-lab.html               # 菲尔兹奖理论 · 动画实验室 (暗色宇宙风格)
...
- **新增** 皮克定理动画课堂（`pick-theorem.html`）：格点多边形面积公式 S = I + B/2 − 1 的交互式动画
  - **第一部分**：动画演示——逐个数内部格点 I 和边界格点 B，用皮克定理计算面积，再用鞋带公式精确验证结果一致
  - **第二部分**：欧拉公式证明——逐步剖分五边形为三角形，展示 V − E + F = 1 不变量，五步推导皮克定理
  - **第三部分**：生活应用题——果农数树（果园面积与果树数量估算）、铺地砖（异形房间地砖预算），均含可展开解答
  - 剪纸风格美术：暖色纸张质感 + 黑色粗边框 + 黄色按钮 + 圆角计数卡片
  - 纯原生 HTML/CSS/JS（Canvas 2D），无外部依赖
- **优化** 化学能转化页面食物选择改为多选模式，选择不同食物时所有属性（能量条、ATP、温度、营养素图表、活动消耗对比线）实时联动更新
- **优化** 数学子页面标签新增「皮克定理」分类

### v2.2.0 (2026-07-26)

- **新增** 文言文填词游戏（`wenyan-fill-game.html`）：2D 绘本风格游戏化文言文语法学习页面
  - **19 篇高中必背经典**，涵盖课内文言文与诗词：
    - 课内经典：劝学（《荀子》）、师说（韩愈）、赤壁赋（苏轼）
    - 史传名篇：鸿门宴（司马迁《史记》）、出师表（诸葛亮）、烛之武退秦师（《左传》）
    - 论说名篇：阿房宫赋（杜牧）、六国论（苏洵）、过秦论（贾谊）、谏太宗十思疏（魏征）、答司马谏议书（王安石）
    - 散文名篇：陈情表（李密）、项脊轩志（归有光）、归去来兮辞（陶渊明）、逍遥游（庄子）
    - 诗歌经典：短歌行（曹操）、琵琶行（白居易）、登泰山记（姚鼐）、子路侍坐（《论语》）
  - **95 道填词题**，每篇 5 题，覆盖四大文言特殊句式：
    - **判断句**：者…也、乃、即、非等标志词
    - **被动句**：于、见、为…所、意念被动等格式
    - **倒装句**：宾语前置（疑问代词前置/否定句代词前置）、状语后置、定语后置
    - **省略句**：省略主语、宾语、介词等
  - 游戏机制：填空选择 + 提示（-5分）+ 显示句式分析 + 语法结构详解 + 白话文翻译 + 典例出处
  - 2D 绘本美术风格：暖色调背景 + 圆角卡片 + 浮动装饰 + 粉/蓝/绿/紫/橙四色句式标签
  - 进度追踪：星级评分系统（⭐⭐⭐）+ 篇目完成状态 + 句式类型掌握度统计
  - 纯原生 HTML/CSS/JS，无外部依赖
- **优化** 统一全部子页面「返回绘本馆」浮动按钮样式与行为
  - 按钮样式统一为圆角胶囊型 + 深色描边 + 浮动阴影（与 `gaokao-chinese-130plus.html` 一致）
  - 点击返回时保持进入页面时的标签页和滚动位置（基于 `localStorage` 存储来源信息）
  - 补全缺失返回按钮的页面：文言文装叉指南、三大变换通信工程应用、从量子纠缠看教育者的角色、导数英雄传
- **优化** 菲尔兹奖理论 · 动画实验室页面内容居中适配屏幕
- **优化** 文言文装叉指南图片路径修正（绝对路径→相对路径）
- **优化** 从量子纠缠看教育者的角色页面适配屏幕比例

### v2.1.0 (2026-07-25)

- **新增** 菲尔兹奖理论 · 动画实验室（`fields-medal-lab.html`）：基于 Kimi 生成页面
  - 二十项菲尔兹奖获奖理论，每项挑选一个最直观的应用做成可动手玩的动画
  - 六大分类：物理宇宙（拓扑绝缘体、规范场、弦论等）、流体与交通（纳维-斯托克斯方程、最优传输）、密码学（椭圆曲线、格密码）、材料相变（相变理论）、AI与影像（蒙日-安培方程、最优传输与GAN）、机器人与经济（刚体控制、纳什均衡）
  - 保留原始暗色宇宙风格（深蓝背景 + 星空粒子 + 金色奖章），仅字体改为二次元风格（ZCOOL KuaiLe + Quicksand）与其他子页面保持一致
  - 主页卡片位于数学页签第一位
- **优化** 首页卡片排序：数学页签以菲尔兹奖实验室为首位；语文页签以高考语文130+提分攻略为首位
- **重构** 主页导航升级为两级标签系统（学科标签 + 分类标签）
