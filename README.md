...
| 页面 | 说明 |
|------|------|
| 三角函数可视化 | 正弦、余弦、正切的图像性质与单位圆联动演示 |
| 指数函数可视化 | 底数 a 对增长/衰减的影响，多场景动画 |
| 对数函数可视化 | 对数运算法则、与指数函数的对称关系 |
| 函数的基本性质 | 定义域、值域、单调性、奇偶性，10 种函数对比 |
| 零点存在定理 | 零点定理条件验证与二分法求近似解 |
| 复指数函数可视化 | 欧拉公式 e^(iθ) 的几何意义与信号处理应用 |

### 一元二次

| 页面 | 说明 |
|------|------|
| 一元二次函数、方程与不等式 | 等式性质、基本不等式、韦达定理、分式不等式、穿根法 |
| 一元二次函数应用案例 | 定价利润、抛体运动、拱桥设计、盈亏平衡、成本约束、药物浓度、污染物安全距离、材料强度、种群模型等 11 个真实案例 |

### 统计与概率

| 页面 | 说明 |
|------|------|
| 计算原理与概率统计 | 蒙特卡洛模拟、概率分布、大数定律、中心极限定理、排列组合 |
| 高中统计实际案例 | 抽样方法、数字特征、回归分析、独立性检验、正态分布 |
| 一本院校理科就业分析 | 985/211 六大专业就业率、升学率、去向可视化 |

### 变换与信号

| 页面 | 说明 |
|------|------|
| 三大变换通信工程应用 (React SPA) | 傅里叶变换、拉普拉斯变换、Z变换在 5G OFDM、滤波器、语音处理中的动画演示 |

### 趣味与创意

| 页面 | 说明 |
|------|------|
| 三角函数实际案例 | 建筑测量、潮汐预测、简谐运动、卫星轨道、音频波形 |
| 文言文装叉指南 (React SPA) | 输入白话文，一键生成文言文金句，179+ 语料库 |
| 从量子纠缠看教育者的角色 (React SPA) | 量子物理视角下的教育隐喻 |
| 古代状元如何看待AI | 王安石、文天祥、苏轼、范仲淹的 AI 策论 |

## 技术栈

- **图表渲染**: [ECharts](https://echarts.apache.org/) 5.5 (SVG 模式)
- **动画**: Canvas 2D (抛体运动等物理模拟)
- **子项目**: React + Vite (文言文指南、量子教育者、三大变换)
